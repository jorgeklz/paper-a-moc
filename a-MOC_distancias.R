    ...
    matriz_exp<-as.matrix(read.table(paste(pathBD,"BD.csv",sep=""), header=T, sep=","))
    #matriz_exp<-normalize(matriz_exp, byrow=TRUE)
    matriz_clases_func<-as.matrix(read.table(paste(pathBD,"BIO.csv",sep=""), header=T, sep=";"))
    numgenes<-nrow(matriz_exp)
    
    ##########################################
    # calcula matriz distancia por Expresi?n
    ##########################################
    if (!file.exists(paste(pathBD,nameBD, "_",nameAlgorithm,"_matriz_distancia_expresion_",nombre_dist_exp,".csv",sep = ""))){
      ...
      write.table(as.matrix(distancia_exp), file = paste(pathBD,nameBD, "_",nameAlgorithm,"_matriz_distancia_expresion_",nombre_dist_exp,".csv",sep = ""),sep = ",",quote =FALSE)
    }else{
      #si ya existe el archivo matriz de distancia expresion lo lee
      distancia_exp<-as.dist(read.table(paste(pathBD,nameBD, "_",nameAlgorithm,"_matriz_distancia_expresion_",nombre_dist_exp,".csv",sep=""), header=T, sep=","))
    }
    ...
    if (!file.exists(paste(pathBD,nameBD, "_",nameAlgorithm,"_matriz_distancia_biologia_",nombre_dist_bio,".csv",sep = ""))){
      ...
      distancia_bio<-as.dist(apply(distancia,1,function(x) (1-(x)))) #1 mas cerca genes, convierto a 0 mas cerca genes
      ...
      distancia_bio<-as.dist(distancia_bio)
      ...
      write.table(distancia_bio, file = paste(pathBD,nameBD, "_",nameAlgorithm,"_matriz_distancia_biologia_",nombre_dist_bio,".csv",sep = ""),sep = ",",quote =FALSE)
      ...
      distancia_bio<-as.dist(read.table(paste(pathBD,nameBD, "_",nameAlgorithm,"_matriz_distancia_biologia_",nombre_dist_bio,".csv",sep=""), header=T, sep=","))
    }
